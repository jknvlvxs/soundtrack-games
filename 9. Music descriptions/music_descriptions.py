...
def run_ollama(video_process:tuple[int, str, list[str]]):
    """
    ...
    try:
        ...
        for descriptions_paths in tqdm(videos_paths, desc=f'Process {pid}'):
        ...
        g_loger.critical(f"Error for video {video_description_path}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_processes', type=int, default=10, help="number of processes to run in parallel setted in OLLAMA_MAX_LOADED_MODELS inside the Ollama container") 
    ...
    with ProcessPoolExecutor(initializer=init_process) as executor:
        executor.map(run_ollama, videos_process_list)

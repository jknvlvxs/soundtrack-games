...
class CLIP(nn.Module):
    def __init__(self, len:int, in_video_dim: int, in_music_dim: int, hidden_dim: int, layers: int, heads: int, drop = 0.45, need_atten: bool = False) -> None:
        ...
        self.video_lnpos = nn.Linear(30, 1)
        self.music_lnpos = nn.Linear(30, 1)
        self.initialize_parameters()
    
    def initialize_parameters(self):
        nn.init.normal_(self.video_linear.weight, std=0.02)
        nn.init.normal_(self.video_lnpos.weight, std=0.02)
        nn.init.normal_(self.music_linear.weight, std=0.02)
        nn.init.normal_(self.music_lnpos.weight, std=0.02)
    ...
    def forward(self, video, music):
        ...
        music_features, music_map = music_atten
        
        video_features = video_features.permute(1, 2, 0)
        music_features = music_features.permute(1, 2, 0)
        
        video_features = self.video_lnpos(video_features)
        music_features = self.music_lnpos(music_features)
        
        # cosine similarity as logits
        logit_scale = self.logit_scale.exp()
        logits_per_video =  logit_scale * video_features[:,:,0] @ music_features[:,:,0].t()#
        logits_per_music = logits_per_video.t()

        return logits_per_video, video_map